...
class PermalinkHelperTest( TestCase ):
    """ Checks helper functions. """

    def setUp(self):
        self.helper = Permalink()
    ...
    def test_make_permalink__resource_not_found(self):
        ...
        data_dct = self.helper.make_permalink( referrer=qdct.get('rfr_id',''), qstring=qstring, scheme='https', host='the_host', path=reverse('findit:findit_base_resolver_url') )
        self.assertEqual(
    ...
    def test_make_permalink__resource_found(self):
        ...
        rsc2.save()
        data_dct = self.helper.make_permalink( referrer=good_referrer, qstring=good_qstring, scheme='https', host='the_host', path=reverse('findit:findit_base_resolver_url') )
        self.assertEqual(
