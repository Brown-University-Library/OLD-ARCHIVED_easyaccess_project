...
class IndexPageLinksTest( TestCase ):
    ...
    def test_article_held_electronically(self):
        ...
        if app_settings.FLY_TO_FULLTEXT is True:
            ...
            self.assertTrue( 'easyAccess (article)' in content )
    ...
    def test_article_chapter_held_at_annex(self):
        ...
        self.assertEqual( 200, response.status_code )
        self.assertInHTML( '<title>easyAccess (article)</title>', content )
        # self.assertInHTML( '<h4 class="easy_what">easyArticle</h4>', content )  // per request, easyArticle branding removed in 2020
...
class GenreEqualsBookItemTest(TestCase):
    ...
    def test_bad_bookitem(self):
        ...
        self.assertEqual( 302, response.status_code )
        log.debug( f'response._headers, ```{pprint.pformat(response._headers)}```' )
        redirect_url = response._headers['location'][1]
