...
class IlliadUrlBuilderTest( TestCase ):
    ...
    def test_enhance_citation_A(self):
        """ Checks that poor bib-dct author data is enhanced with `rft.creator` data if possible. """
    ...
    def test_enhance_citation_B(self):
        """ Checks that poor bib-dct author data is enhanced with `rft.au` data if possible. """
        querystring = 'ctx_ver=Z39.88-2004&ctx_enc=info:ofi/enc:UTF-8&rfr_id=info:sid/ProQuest+Dissertations+%26+Theses+Global&rft_val_fmt=info:ofi/fmt:kev:mtx:dissertation&rft.genre=dissertations+%26+theses&rft.jtitle=&rft.atitle=&rft.au=Yu%2C+Xingfeng&rft.aulast=Yu&rft.aufirst=Xingfeng&rft.date=2018-01-01&rft.volume=&rft.issue=&rft.spage=&rft.isbn=9780438852358&rft.btitle=&rft.title=Effectiveness+of+a+Nurse-Led+Integrative+Health+and+Wellness+%28NIHaW%29+Programme+on+Behavioural%2C+Psychosocial+and+Biomedical+Outcomes+among+Individuals+with+Newly+Diagnosed+Type+2+Diabetes%3A+A+Randomised+Controlled+Trial&rft.issn=&rft_id=info:doi/'
        initial_bib_dct = {
            '_openurl': 'ctx_ver=Z39.88-2004&rft_val_fmt=info%3Aofi/fmt%3Akev%3Amtx%3Ajournal&rft.atitle=Unknown&rft.genre=article&rfr_id=info%3Asid/info%3Asid/ProQuest+Dissertations+%26+Theses+Global&rft.date=2018&rft.isbn=9780438852358',
            '_rfr': 'info:sid/ProQuest Dissertations & Theses Global',
            '_valid': False,
            'identifier': [{'id': '9780438852358', 'type': 'isbn'}],
            'journal': {'name': 'Not provided'},
